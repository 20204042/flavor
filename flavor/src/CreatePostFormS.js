...
function CreatePostFormS() {
  const [newPost2, setNewPost] = useState({ title: '', content: '' });
  const [error, setError] = useState('');
  const navigate = useNavigate();

  const handleCreatePost = async () => {
    try {
      if (!newPost2.title || !newPost2.content) {
      ...
      const contentWithBr = newPost2.content.replace(/\n/g, '<br>');
      ...
      const response = await fetch('http://localhost:3001/suggestlist/createpostforms', {
        method: 'POST',
        headers: {
    ...
    <div>
      ...
      <div className="write">
        ...
        <input
          type="text"
          placeholder="제목"
          value={newPost2.title}
          onChange={(e) => setNewPost({ ...newPost2, title: e.target.value })}
        />
        <textarea
          placeholder="내용"
          value={newPost2.content}
          onChange={(e) => setNewPost({ ...newPost2, content: e.target.value })}
          rows="10"
        />
